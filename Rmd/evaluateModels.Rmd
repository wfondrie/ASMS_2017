...
cAbRes$ex <- "Ab Resistant"
cAbSus$ex <- "Ab Susceptible"
cKpRes$ex <- "Kp Resistant"
cKpSus$ex <- "Kp Susceptible"
cAbRes <- rename(cAbRes, coeff = AbResCoeff)
cAbSus <- rename(cAbSus, coeff = AbSusCoeff)
cKpRes <- rename(cKpRes, coeff = KpResCoeff)
cKpSus <- rename(cKpSus, coeff = KpSusCoeff)
...
coeffSumm <- coeff %>%
  group_by(ex) %>%
  summarize(Sen_100 = sum(pred[coeff == 1] == "pos") / length(pred[coeff == 1]),
            Sen_90 = sum(pred[coeff > 0.9] == "pos") / length(pred[coeff > 0.9]),
            Sen_80 = sum(pred[coeff > 0.8] == "pos") / length(pred[coeff > 0.8]),
            Sen_70 = sum(pred[coeff > 0.7] == "pos") / length(pred[coeff > 0.7]),
            Sen_60 = sum(pred[coeff > 0.6] == "pos") / length(pred[coeff > 0.6]),
            Sen_50 = sum(pred[coeff > 0.5] == "pos") / length(pred[coeff > 0.5]),
            Sen_40 = sum(pred[coeff > 0.4] == "pos") / length(pred[coeff > 0.4]),
            Sen_30 = sum(pred[coeff > 0.3] == "pos") / length(pred[coeff > 0.3]),
            Sen_20 = sum(pred[coeff > 0.2] == "pos") / length(pred[coeff > 0.2]),
            Sen_10 = sum(pred[coeff > 0.1] == "pos") / length(pred[coeff > 0.1]),
            Sen_0 = sum(pred[coeff > 0] == "pos") / length(pred[coeff > 0])) %>%
  gather(`Weight Threshold`, Sensitivity, -ex) %>%
  mutate(`Weight Threshold` = as.numeric(gsub("Sen_", "", `Weight Threshold`)))

coeffSumm %>%
  ggplot(aes(x = `Weight Threshold`, y = Sensitivity, group = ex, color = ex)) +
  geom_line() +
  geom_point() +
  coolTheme +
  theme(legend.title = element_blank())

ggsave("results/complexWeights.png", width = 5, height = 4)
```
